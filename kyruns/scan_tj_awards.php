		...
		<div class="col-md-7" style="background-color:rgb(255,255,255);"><!-- 表单循环输出  begin -->
		    <div class="row-fluid" style="padding-top:18px;">
				<div class="span12 text-center fontTitle">
					<span style="font-size:28px;">信</span><h3 style="display:inline;">息科学与工程学院科研获奖统计表</h3>
				</div>
			</div>
				...
				<form name="form3" method="get">
				    <table class="table table-bordered table-hover">
						...
						<?php
							...
							if($select_date==""){
								$sql = "SELECT * FROM tb_tj_awards WHERE uid = {$_SESSION['uid']} order by id desc LIMIT $startPage,$pageSize";
								...
								$sql = "SELECT * FROM tb_tj_awards WHERE uid = {$_SESSION['uid']} AND date = {$select_date} order by id desc LIMIT $startPage,$pageSize";
								$result = mysqli_query($conn,$sql);
								$row_num = mysqli_num_rows($result);
								/* if($row_num==0){
									$select_date = "";
							...
							<tr class="<?php echo $col;?>" >
								<td><input type="radio" name="awards_id" value="<?php echo $id;?>"/></td>
				                <td>
				                    <a class="text-link" href="javascript:void(0)" onclick="javascript:window.open('read_awards.php?note_id=<?php echo $id;?>','_blank','toolbar=0, location=0, directories=0, status=0, menubar=0, scrollbars=1, resizable=0, titlebar=1, width=800, height=400, left=288, top=184')"><img src="img/more.png">&nbsp;<?php echo $prize;?></a>     
								</td>
			...
			<div class="row" style="padding-top:15px;padding-right:50px; padding-bottom:30px;">
				...
				<?php 
					...
					//显示分页条
					//获取数据总条数
					$total_sql = "SELECT COUNT(*) FROM tb_tj_awards WHERE uid = {$_SESSION['uid']}";
					...
					$page_banner .= '<form action="scan_tj_awards.php" method="get" style="display:inline;">';
	...
	<script>
		function del(){
			if(confirm("此操作会删除选中项，是否继续？")){
				document.form3.action="do_tj_awards.php";
				}
			}
		function mod(){
			document.form3.action="modify_tj_awards.php";
			}
		function look(){
			document.formyear.action="scan_tj_awards.php";
			}
		function out(){
			document.formyear.action="modify_tj_awards.php";
			}
    </script>

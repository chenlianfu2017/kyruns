...
<div id="wrapper">
							...
							<table class="table table-hover table-condensed" style="padding: 8px;height:1;padding: 5px;vertical-align: top;border-top: 1px solid #ddd">
								...
								<?php
									...
									if($select_author && $select_date <> ""){
										$sql = "SELECT * FROM `tb_tj_competition` WHERE wname = '$select_author' AND date = {$select_date} ORDER BY id DESC LIMIT $startPage,$pageSize";
									}elseif($select_date == ""){
										if($select_author == "" ){
											$sql = "SELECT * FROM `tb_tj_competition` ORDER BY id DESC LIMIT $startPage,$pageSize";
										}else{
											$sql = "SELECT * FROM `tb_tj_competition` WHERE wname = '$select_author' ORDER BY id DESC LIMIT $startPage,$pageSize";
										}
									}elseif($select_author == ""){
										if($select_date == "" ){
											$sql = "SELECT * FROM `tb_tj_competition` ORDER BY id DESC LIMIT $startPage,$pageSize";
										}else{
											$sql = "SELECT * FROM `tb_tj_competition` WHERE date = {$select_date} ORDER BY id DESC LIMIT $startPage,$pageSize";
										}
									}
									...
									<tr class="<?php echo $col;?>" style="vertical-align:top;border-top: 1px solid #ddd;vertical-align:bottom;border-bottom: 2px solid #ddd">
						                ...
						                <td style="padding:8px;line-height:0.5;vertical-align:top;border-top:1px solid #ddd"><!-- 在标题上添加连接 -->
					                    	<a href="javascript:void(0)" title="点击查看详情" onclick="javascript:window.open('read_competition.php?note_id=<?php echo $id;?>','_blank','toolbar=0, location=0, directories=0, status=0, menubar=0, scrollbars=1, resizable=0, titlebar=1, width=800, height=420, left=288, top=184')" style="text-decoration:none;"><?php echo $wname;?></a>    
						                </td>
						                <td style="padding:8px;line-height:0.5;vertical-align:top;border-top:1px solid #ddd"><?php echo $sponsor;?></td>
			...
			<div class="panel-body no-padding">
			    ...
			    <?php 
				    ...
				    if($select_date == "" && $select_author == ""){
				    	$total_sql = "SELECT COUNT(*) FROM `tb_tj_competition`";
				    }elseif($select_date == "" && $select_author != ""){
				    	$total_sql = "SELECT COUNT(*) FROM `tb_tj_competition` WHERE wname = '$select_author'";
				    }elseif($select_date != "" && $select_author == ""){
				    	$total_sql = "SELECT COUNT(*) FROM `tb_tj_competition` WHERE date = '$select_date'";
				    }elseif($select_date != "" && $select_author != ""){
				    	$total_sql = "SELECT COUNT(*) FROM `tb_tj_competition` WHERE date = '$select_date' AND wname = '$select_author'";
				    }
					...
					$page_banner = "";
					if($page>1){
						$page_banner.="<li><a href='".$_SERVER['PHP_SELF']."?p=1.$select_year'>首页</a></li>";
						$page_banner.="<li><a href='".$_SERVER['PHP_SELF']."?p=".($page-1).$select_year."'>上一页</a></li>";
					}else{
					...
					for($i = $start;$i<=$end;$i++){
						$page_banner.="<li><a href='".$_SERVER['PHP_SELF']."?p=".$i.$select_year."'>{$i}</a><li>";
					}
					//尾部省略
					...
					if($page<$total_pages){
						$page_banner.="<li><a href='".$_SERVER['PHP_SELF']."?p=".($page+1).$select_year."'>下一页</a></li>";
						$page_banner.="<li><a href='".$_SERVER['PHP_SELF']."?p=".($total_pages).$select_year."'>尾页</a></li>";
					}else{
	...
	<script>
		...
		function out(){
			document.formyear.action="export_tj_competition.php";
			}
	</script>
</body>
</html>

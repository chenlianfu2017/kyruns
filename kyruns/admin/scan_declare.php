				            ...
				            <form name="form1" method="get">
				                <table class="table table-hover table-condensed" style="padding: 8px;height:1;padding: 5px;vertical-align: top;border-top: 1px solid #ddd">
						           ...
						           <tbody><!--tbody中为要循环输出的内容  -->
									...
									if($time == ""){
										 if($manname == "" ){
										 	$sql = "SELECT * FROM tb_declare ORDER BY did DESC LIMIT $startPage,$pageSize";
										 }else{
										 	$sql = "SELECT * FROM tb_declare WHERE dmanname = '$manname' ORDER BY did DESC LIMIT $startPage,$pageSize";
										 }
									}else{
									    if($manname == "" ){
										    $sql = "SELECT * FROM tb_declare WHERE dtime = '$time' ORDER BY did DESC LIMIT $startPage,$pageSize";
										 }else{
										 	$sql = "SELECT * FROM tb_declare WHERE dmanname = '$manname' AND dtime = '$time' ORDER BY did DESC LIMIT $startPage,$pageSize";							 	
										 }
									}
									...
									for($i=0;$i<$row_num;$i++){
										...
										<tr class="<?php echo $col;?>" style="vertical-align:top;border-top: 1px solid #ddd;vertical-align:bottom;border-bottom: 2px solid #ddd">
							                ...
							                <?php
							                	if($dstatus == 1){?>
								                	<a href ="scan_declare.php?did=<?php echo $did;?>&decision=1"><?php echo "同意";?>&nbsp;|</a>
								                	<a href ="scan_declare.php?did=<?php echo $did;?>&decision=0"><?php echo "拒绝";?></a>
							               <?php }else{
								                	if($dstatus==0){
								                		echo "已拒绝";
								                	}else{
			        ...
			        <div  class="panel panel-info" data-widget="{&quot;draggable&quot;: &quot;false&quot;}" data-widget-static="">
			            <div class="panel-body no-padding">
				            <nav class="pull-right">
								...
								//获取数据总条数
								if($time == "" && $manname == ""){
									$total_sql = "SELECT COUNT(*) FROM tb_declare";
								}elseif($time == "" && $manname != ""){
									$total_sql = "SELECT COUNT(*) FROM tb_declare WHERE dmanname = '$manname'";
								}elseif($time != "" && $manname == ""){
								    $total_sql = "SELECT COUNT(*) FROM tb_declare WHERE dtime = '$time'";
								}elseif($time != "" && $manname != ""){
									$total_sql = "SELECT COUNT(*) FROM tb_declare WHERE dtime = '$time' AND dmanname = '$manname'";
								}

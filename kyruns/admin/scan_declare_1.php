			    ...
			    <div class="row" style="padding-left:40px; padding-right:50px;">
				    ...
				    <div class="panel panel-info" data-widget="{&quot;draggable&quot;: &quot;false&quot;}" data-widget-static="">
				        <div class="panel-body no-padding">			
							...
							if($time == ""){
								 if($manname == "" ){
								 	 $sql = "SELECT * FROM tb_declare WHERE dtype = '科研项目' ORDER BY did DESC LIMIT $startPage,$pageSize";
								 }else{
								 	 $sql = "SELECT * FROM tb_declare WHERE dtype = '科研项目' AND dmanname = '$manname' ORDER BY did DESC LIMIT $startPage,$pageSize";							 	
								 }
							}else{
							    if($manname == "" ){
								 	 $sql = "SELECT * FROM tb_declare WHERE dtype = '科研项目' AND dtime = '$time' ORDER BY did DESC LIMIT $startPage,$pageSize";
								 }else{
								 	 $sql = "SELECT * FROM tb_declare WHERE dtype = '科研项目' AND dmanname = '$manname' AND dtime = '$time' ORDER BY did DESC LIMIT $startPage,$pageSize";
								 }							 
							}
							...
							for($i=0;$i<$row_num;$i++){
								...
								<tr class="<?php echo $col;?>" style="vertical-align:top;border-top: 1px solid #ddd;vertical-align:bottom;border-bottom: 2px solid #ddd">
					                ...
					                <?php
					                	if($dstatus == 1){?>
						                	<a href ="scan_declare_1.php?did=<?php echo $did;?>&decision=1"><?php echo '同意';?>&nbsp;|</a>
						                	<a href ="scan_declare_1.php?did=<?php echo $did;?>&decision=0"><?php echo '拒绝';?></a>
					               <?php }else{
						                	if($dstatus==0){
						                		echo "已拒绝";
						                	}else{
			        ...
			        <div  class="panel panel-info" data-widget="{&quot;draggable&quot;: &quot;false&quot;}" data-widget-static="">
			            <div class="panel-body no-padding">
				            <nav class="pull-right">
								...
								//获取数据总条数
								if($time == "" && $manname == ""){
									$total_sql = "SELECT COUNT(*) FROM tb_declare WHERE dtype = '科研项目'";
								}elseif($time == "" && $manname != ""){
									$total_sql = "SELECT COUNT(*) FROM tb_declare WHERE dmanname = '$manname' AND dtype = '科研项目'";
								}elseif($time != "" && $manname == ""){
								    $total_sql = "SELECT COUNT(*) FROM tb_declare WHERE dtime = '$time' AND dtype = '科研项目'";
								}elseif($time != "" && $manname != ""){
									$total_sql = "SELECT COUNT(*) FROM tb_declare WHERE dtime = '$time' AND dmanname = '$manname' AND dtype = '科研项目'";
								}
...
<script>
	function look(){
		document.formyear.action="scan_declare_1.php";
		}
	function out(){

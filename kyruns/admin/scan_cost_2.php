...
<body>
				        ...
				        <div class="panel-body no-padding">		
				        	<form name="form1" method="get">
				            	<table class="table table-hover table-condensed" style="padding: 8px;height:1;padding: 5px;vertical-align: top;border-top: 1px solid #ddd" >
		                            ...
		                            if($time == ""){//不限制年份
										 if($manname == "" ){//不限制负责人
										        $sql = "SELECT * FROM tb_cost WHERE ctype = '论文' ORDER BY cid DESC LIMIT $startPage,$pageSize";
										 }else{//限制负责人
										 		$sql = "SELECT * FROM tb_cost WHERE cmanname = '$manname' AND ctype = '论文'  ORDER BY cid DESC LIMIT $startPage,$pageSize";
										 }
									}else{//限制年份
									    if($manname == "" ){//不限制负责人
										        $sql = "SELECT * FROM tb_cost WHERE ctime = '$time' AND ctype = '论文'  ORDER BY cid DESC LIMIT $startPage,$pageSize";
										 }else{//限制年份 限制负责人
										 		$sql = "SELECT * FROM tb_cost WHERE cmanname = '$manname' AND ctime = '$time' AND ctype = '论文'  ORDER BY cid DESC LIMIT $startPage,$pageSize";
										 }
									}
									...
									for($i=0;$i<$row_num;$i++){
										...
										<tr class="<?php echo $col;?>"  style="vertical-align:top;border-top: 1px solid #ddd;vertical-align:bottom;border-bottom: 2px solid #ddd">
							                ...
							                <?php
							                	if($cstatus == 1){?>
								                	<a href ="scan_cost_2.php?cid=<?php echo $cid;?>&decision=1"><?php echo '同意';?>&nbsp;|</a>
								                	<a href ="scan_cost_2.php?cid=<?php echo $cid;?>&decision=0"><?php echo '拒绝';?></a>
							                <?php }else{
								                       if($cstatus==0){
								                		   echo "已拒绝";
								                	   }else{
			        ...
			        <div  class="panel panel-info" data-widget="{&quot;draggable&quot;: &quot;false&quot;}" data-widget-static="">
			            <div class="panel-body no-padding">
				            <nav class="pull-right">
								...
								//获取满足条件的数据总条数
			                    if($time == "" && $manname == "" ){
									$total_sql = "SELECT COUNT(*) FROM tb_cost WHERE ctype = '论文'";
								}elseif($time == "" && $manname != ""){
									$total_sql = "SELECT COUNT(*) FROM tb_cost WHERE cmanname = '$manname' AND ctype = '论文'";
								}elseif($time != "" && $manname == ""){
								    $total_sql = "SELECT COUNT(*) FROM tb_cost WHERE ctime = '$time' AND ctype = '论文'";
								}elseif($time != "" && $manname != ""){
									$total_sql = "SELECT COUNT(*) FROM tb_cost WHERE ctime = '$time' AND cmanname = '$manname' AND ctype = '论文'";
								}
	...
	<script>
		function look(){
			document.formyear.action="scan_cost_2.php";
			}
		function out(){

...
<html lang="zh-CN">
					...
					<form  name="form1" method="get">
						<div class="form-group" style="padding:0 30px 0">
							...
							<select class="form-control input-sm" id="select" name="dtype">
								<option value="<?php echo $_GET['dtype']?>"><?php if($_GET['dtype']==''){echo "所有类别";}else{echo $_GET['dtype'];}?></option>
						...
						<div class="form-group" style="padding:0 30px 0">
							...
							<select class="form-control input-sm" id="year1" name="dtime">
								<option value="<?php echo $_GET['dtime']?>"><?php if($_GET['dtime']==''){echo "所有年份";}else{echo $_GET['dtime'];}?></option>
			...
			<div class="col-md-7" style="background-color: rgb(255, 255, 255);">				
				 <div class="row-fluid" style="padding-top:18px;">
					<div class="span12 text-center fontTitle">
						...
						<h3 style="display:inline">息科学与工程学院项目申报统计表</h3>
					</div>
				 </div>
					...
					<form name="form2" method="get">
						<table class="table table-bordered table-hover">
							...
							<!--tbody中为要循环输出的内容  -->
								...
								//取出数据
								$select_date = $_GET['dtime'];
								$ttype = $_GET['dtype'];
								
								if($ttype != ""){//如果限制类型
									
									if( $select_date == ""){//不限制年份								
									    $sql = "SELECT * FROM tb_declare WHERE uid = {$_SESSION['uid']} AND dtype = '$ttype' ORDER BY did DESC LIMIT $startPage,$pageSize";
									 
									}else{//限制年份								
									    $sql = "SELECT * FROM tb_declare WHERE uid = {$_SESSION['uid']} AND dtype = '$ttype' AND dtime = {$select_date} ORDER BY did DESC LIMIT $startPage,$pageSize";
									}
								}else{//不限制类型
									if( $select_date == ""){//不限制年份									
										$sql = "SELECT * FROM tb_declare WHERE uid = {$_SESSION['uid']} ORDER BY did DESC LIMIT $startPage,$pageSize";
									}else {//限制年份
										$sql = "SELECT * FROM tb_declare WHERE uid = {$_SESSION['uid']} AND dtime = {$select_date} ORDER BY did DESC LIMIT $startPage,$pageSize";
									}							
								}
								...
								<tr class="<?php echo $col;?>" >
									<td><input type="radio" name="did" value="<?php echo $did;?>" /></td>
									<td><!-- 在标题上添加连接 -->
									<a class="text-link" href="javascript:void(0)" onclick="javascript:window.open('read_declare.php?note_id=<?php echo $did;?>','_blank','toolbar=0, location=0, directories=0, status=0, menubar=0, scrollbars=1, resizable=0, titlebar=1, width=900, height=500, left=288, top=184')"><img src="img/more.png">&nbsp;<?php echo $dproname?></a></td>
									<td><?php echo $dnumber;?></td>
									<td><?php echo $dmanname;?></td>
									<td><?php echo $dtime;?></td>
									<!-- 根据dstatus的值输出不同的颜色 1：显示红色等待审核 2：显示绿色通过审核-->								
									<td style="<?php if($dstatus == 1){echo 'color:blue;';}elseif($dstatus == 2){echo 'color:green';}elseif($dstatus == 0){echo 'color:red';}?>">
									           <?php if($dstatus == 1){echo "等待审核";}elseif($dstatus == 2){ echo "通过审核";}elseif($dstatus == 0){ echo "未通过";}?>
									</td>
									<td><?php echo $dmoney;?></td>
				...
				<div class="row" style="padding-top:15px; padding-right:50px; padding-bottom:30px;">
			        <nav class="pull-right">
				        <ul class="pagination fontPage">
							...
							if($ttype == ""){						
							   if($select_date == ""){
							       $total_sql = "SELECT COUNT(*) FROM tb_declare WHERE uid = '$_SESSION[uid]'";
							    }else{
								   $total_sql = "SELECT COUNT(*) FROM tb_declare WHERE uid = '$_SESSION[uid]' AND dtime = '$select_date'";   
							    }					    
							}else{
								if($select_date == ""){
									$total_sql = "SELECT COUNT(*) FROM tb_declare WHERE uid = '$_SESSION[uid]' AND dtype = '$ttype'";
								}else{
									$total_sql = "SELECT COUNT(*) FROM tb_declare WHERE uid = '$_SESSION[uid]' AND dtype = '$ttype' AND dtime = '$select_date'";
								}
							}
	...
	<script>
		...
		function out(){
			document.form1.action="export_declare.php";
			}
		function del(){
			if(confirm("此操作会删除选中项，是否继续？")){
				document.form2.action="do_declare.php";
				}
			}
		function mod(){
			document.form2.action="modify_declare.php";
			}
	</script>
